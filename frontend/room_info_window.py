...
class RoomInfoWindow(QtWidgets.QDialog):
    ...
    def load_data(self):
        """Загрузка данных о номере через API"""
        try:
            self.show_loading(True)
            room_data = self.api_client.get_room_full_info(self.room_number)

            self.fill_basic_info(room_data.get("room_info", {}))
            self.fill_bookings_table(room_data.get("bookings", []))
            self.fill_service_table(
                room_data.get("cleaning", []),
                room_data.get("services", [])
            )
            self.fill_payments_table(room_data.get("payments", []))

        except Exception as e:
            QtWidgets.QMessageBox.warning(self, "Ошибка", f"Не удалось загрузить данные: {str(e)}")
        finally:
